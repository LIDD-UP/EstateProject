...
class RealtorAppSpider(scrapy.Spider):
    name = 'realtor_app'
    allowed_domains = ['realtor.com', 'mapi-ng.rdc.moveaws.com']
    start_urls = ['http://realtor.com/']

    def __init__(self,
                 start_urls=None, user_agent_list=None,scrapy_start_time=None,
                 *args, **kwargs):
        super(RealtorAppSpider, self).__init__(*args, **kwargs)
        print(start_urls)
    ...
    def parse(self, response):
        ...
        for house in houses:
            ...
            if len(x) != 0:
                ...
                x = x.replace('-', '')
                print(x)
            next_detail_page_url = "https://mapi-ng.rdc.moveaws.com/api/v1/properties/{}?prop_status=for_sale&schema=legacy&client_id=rdc_mobile_native%2C9.3.7%2Candroid".format(x)
            yield scrapy.Request(url=next_detail_page_url,callback=self.parse_content,headers={
