...
from AmericanRealEstate.items import RealtorDetailPageJsonWebItem
from AmericanRealEstate.settings import realtor_search_criteria, realtor_domain_url


class RealtorWSpider(RedisSpider):
    name = 'realtor_w'
    ...
    def __init__(self,
                 ...
                 *args, **kwargs):
        super(RealtorWSpider, self).__init__(*args, **kwargs)
        ...
        print('user_agent个数：', len(self.user_agent_list))
    ...
    custom_settings = {
        ...
        "DOWNLOADER_MIDDLEWARES":{
            ...
            'AmericanRealEstate.middlewares.RealtorDetailPageWMiddleware': 1,
            'AmericanRealEstate.middlewares.RealtorDetailPageWProcessUrlMiddleware': 545,
        },
    ...
    def parse(self,response):
        # 接口的parse
        realtor_detail_pageJson_item = RealtorDetailPageJsonWebItem()
        realtor_detail_pageJson_item['detailJson'] = response.text
        realtor_detail_pageJson_item['propertyId'] = re.search(r'\d+',response.url).group()
